...
public class UIEquipmentListElement : MonoBehaviour
{
	...
	public void LevelUpItem()
	{
		unitDetail.LevelUpItem(this.item, (item) =>
		{
			...
			{
				// the level up item cost is hardcoded, both in value and which currency does it consume
				GlobalUserData.Instance.AddCurrency("Gold", -(int)Math.Pow(this.item.level, 2));
